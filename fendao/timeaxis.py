...
clanbattlework_path = os.path.join(
    os.path.dirname(__file__), 'clanbattlework.json')
        ...
        for work in data["data"]:
            ...
            for bosswork in work["homework"]:
                clanbattle_work[boss_id][stage]["bosswork"][bosswork["sn"]] = {
                    "info": bosswork["info"], "unit_id": bosswork["unit"], "damage": bosswork["damage"], "video_link": bosswork["video"]}
...
async def letter2stageid(stage_letter):
    for stage in stage_dict:
        if stage_letter == stage:
            return str(stage_dict[stage])
    return "无效stage"
...
async def get_info(stage, boss=None, work_id=None, type="TWS"):
    ...
    if not stage.isdigit():
        stage = await letter2stageid(stage.upper())
